...
function resolveArtifactRoot() {
    ...
    const looksLikeWindowsPath = /^[a-zA-Z]:[/\\]/.test(configured) || configured.includes("\\");
    if (looksLikeWindowsPath && process.platform !== "win32") {
        console.warn(`[indexer] CONTRACT_ARTIFACTS_DIR is a Windows path (${configured}); using ${defaultArtifactRoot}`);
        return defaultArtifactRoot;
    }
