...
async function main() {
    ...
    console.log(`airdrop keeper ready: chain=${airdropKeeperConfig.chainId}, manager=${registry.pumpAirdropManager}, keeper=${account.address}`);
    while (!shuttingDown) {
        ...
        if (airdropKeeperConfig.once)
            break;
        await sleep(airdropKeeperConfig.pollIntervalMs);
    }
}
async function listPendingAirdrops() {
...
async function finalizeOne(client, manager, airdrop) {
    ...
    const holds = await snapshotHoldsAtQualifyEnd(airdrop, filtered.map((c) => c.address));
    const qualified = filtered
        ...
        if (!holdOk || !buyOk)
            return null;
        return { address: address, holdWei };
    })
        .filter((row) => row !== null)
        .sort((a, b) => (a.holdWei === b.holdWei ? 0 : a.holdWei > b.holdWei ? -1 : 1))
        .slice(0, 100);
    if (qualified.length === 0) {
        console.warn(`airdrop ${airdrop.id}: no qualified winners, marking CLOSED in DB (sweep full reward via admin after claim window)`);
    ...
    console.log(`finalized airdrop ${airdrop.on_chain_id}: winners=${qualified.length}, root=${root}, tx=${hash}`);
}
async function loadCandidates(airdrop, minHoldWei, minBuyBnbWei) {
    const result = await pools.launchpad.query(`
      WITH buy_volume AS (
...
async function filterSocialGate(airdropId, addresses) {
    const required = await pools.launchpad.query(`
...
async function snapshotHoldsAtQualifyEnd(airdrop, addresses) {
    const holds = new Map();
