...
export function incrementalBoardStatsEnabled() {
...
export async function seedBoardStatsOnTokenCreated(db, seed) {
...
export async function upsertBoardStatsAfterTrade(client, update) {
    if (!incrementalBoardStatsEnabled())
        return;
    const mcap = Number(update.marketCapZug);
    const spot = Number(update.spotPriceZug);
    const netVol = Number(update.tradeNetZug);
    const volAdd = Number.isFinite(netVol) && netVol > 0 && isWithinLast24Hours(update.blockTime)
        ? netVol
        : 0;
    const traderCount = await client.query(`
...
export async function reconcileBoardStatsRollingWindows(pool) {
    if (!incrementalBoardStatsEnabled())
        return;
    await pool.query(`
...
export function marketCapZugFromSpot(spotPriceZug) {
    const spot = Number(spotPriceZug);
    if (!Number.isFinite(spot) || spot <= 0)
        return "0";
    return String(spot * TOKEN_SUPPLY);
}
export async function readBoardStatsForPublish(db, tokenAddress) {
    if (!incrementalBoardStatsEnabled())
        return null;
...
function isWithinLast24Hours(blockTime) {
