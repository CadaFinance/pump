...
export async function getTopMcapToken(pool) {
...
async function getActiveKingToken(pool) {
    const result = await pool.query(`
...
async function dethroneActiveKing(pool, blockTime) {
    await pool.query(`
      UPDATE king_history
      SET dethroned_at = $1
      WHERE dethroned_at IS NULL
...
async function awardKingMission(ctx, creatorAddress, tokenAddress, txHash, blockTime, marketCapBnb) {
 ...
 */
export async function recomputeKing(ctx, blockTime, txHash) {
    ...
    if (activeKingToken === top.tokenAddress) {
        await awardKingMission(ctx, top.creatorAddress, top.tokenAddress, txHash, blockTime, top.marketCapBnb);
        return;
    }
...
async function getTokenMcap(pool, tokenAddress) {
    const result = await pool.query(`
      SELECT
        t.address AS token_address,
        t.creator_address,
    ...
    `, [tokenAddress]);
    const row = result.rows[0];
    if (!row || Number(row.trade_count) <= 0)
        return null;
    return {
        tokenAddress: row.token_address,
        creatorAddress: row.creator_address,
...
async function getActiveKingMcap(pool) {
    const result = await pool.query(`
      SELECT
        kh.token_address,
        COALESCE(b.market_cap_zug, 0)::text AS market_cap_zug
...
export async function recomputeKingAfterTrade(ctx, blockTime, txHash, tradedTokenAddress) {
