...
async function main() {
    ...
    try {
        ...
        const txResult = await pools.launchpad.query("SELECT launch_tx_hash FROM tokens WHERE address = $1", [top.tokenAddress]);
        const txHash = (txResult.rows[0]?.launch_tx_hash ?? "0x" + "0".repeat(64));
        console.log(`Current #1: ${top.tokenAddress} (creator ${top.creatorAddress}, mcap ${top.marketCapBnb} BNB)`);
        await recomputeKing({ launchpadPool: pools.launchpad, pointsBridge }, new Date(), txHash);
