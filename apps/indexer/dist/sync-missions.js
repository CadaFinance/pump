...
async function main() {
    ...
    if (!pools.vm1) {
        throw new Error("VM1_MAIN_DB_URL is required — set it to the same pump_db URL as LAUNCHPAD_DATABASE_URL on this VM.");
    }
    ...
    try {
      ...
      `);
        for (const token of tokens.rows) {
            const txHash = token.launch_tx_hash;
      ...
      `);
        for (const trade of trades.rows) {
            ...
            if (trade.side === "BUY" && zugBnb >= Number(FIRST_SMART_BUY_MIN_WEI) / 1e18) {
                const creatorResult = await pools.launchpad.query("SELECT creator_address FROM tokens WHERE address = $1", [trade.token_address]);
        ...
        const volumes = await pools.launchpad.query("SELECT address, total_volume_zug::text FROM user_volumes");
        for (const row of volumes.rows) {
            if (Number(row.total_volume_zug) < VOLUME_MONSTER_MIN_BNB)
                continue;
            await pointsBridge.award({
                address: row.address,
        ...
        const topTx = await pools.launchpad.query(`
        SELECT launch_tx_hash FROM tokens
        WHERE is_hidden = false
        ORDER BY created_at DESC
        LIMIT 1
      `);
        const kingTx = (topTx.rows[0]?.launch_tx_hash ?? `0x${"0".repeat(64)}`);
        await recomputeKing({ launchpadPool: pools.launchpad, pointsBridge }, new Date(), kingTx);
        console.log(`Mission sync complete: deploy=${deployCount}, dailySwap=${swapCount}, smartBuy=${smartBuyCount}, volumeMonster=${volumeCount}, king=recomputed`);
