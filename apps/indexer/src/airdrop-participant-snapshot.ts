...
} from "./airdrop-qualify-volume.js";

type Queryable = Pool | PoolClient;

type AirdropRulesRow = {
  onchain?: {
    minHoldWei?: string;
    minBuyBnbWei?: string;
  };
...
function weiStringToDecimal(wei: string): number {
...
export async function refreshParticipantSnapshotIndexer(
  db: Queryable,
  airdropId: string,
  address: string
): Promise<void> {
  ...
  const requiredTasks = await db.query<{ count: string }>(
    `
  ...
  const doneTasks = await db.query<{ count: string }>(
    `
  ...
  const participant = await db.query<{ social_gate_passed_at: Date | null }>(
    `
  ...
  const allocation = await db.query<{ rank: number; amount: string }>(
    `
...
export async function syncAllocationSnapshotsIndexer(
  db: Queryable,
  airdropId: string
): Promise<void> {
  const winners = await db.query<{ address: string }>(
    `SELECT address FROM airdrop_allocations WHERE airdrop_id = $1::bigint`,
    [airdropId]
  );
...
export async function markParticipantClaimedIndexer(
  db: Queryable,
  airdropId: string,
  address: string,
  claimedAt: Date
