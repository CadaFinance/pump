...
/** BondingCurveManager.curves(token) return tuple. */
type CurveTuple = readonly [
  Address,
  Address,
  bigint,
  bigint,
...
async function listDbCandidates(): Promise<Address[]> {
...
async function tick(bondingCurveManager: Address): Promise<void> {
  ...
  for (const token of candidates) {
    try {
      ...
      console.log(
        `graduating ${token} reserve=${curve.reserveZug.toString()} target=${curve.targetZug.toString()} triggered=${curve.graduationTriggered}`
      );
...
async function main(): Promise<void> {
  ...
  console.log(
    `graduation keeper ready: chain=${keeperConfig.chainId}, keeper=${account.address}, bondingCurve=${registry.bondingCurveManager}`
  );

  do {
    await tick(registry.bondingCurveManager);
    if (keeperConfig.once) break;
