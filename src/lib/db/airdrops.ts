...
export async function getActiveAirdropForLinkedToken(
  linkedToken: string
): Promise<AirdropListItem | null> {
  const pool = getLaunchpadPool();
  ...
  const result = await pool.query<{
    ...
    `
      ...
      ORDER BY
        CASE
          WHEN a.status = 'ACTIVE' AND a.qualify_end > now() THEN 0
          WHEN a.status = 'FINALIZED' AND (a.claim_end IS NULL OR a.claim_end > now()) THEN 1
          WHEN a.qualify_start > now() THEN 2
          ELSE 3
        END,
        a.qualify_end DESC
      LIMIT 1
